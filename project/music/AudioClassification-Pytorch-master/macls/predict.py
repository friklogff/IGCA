...
class MAClsPredictor:
    ...
    def mypredict(self, audio_data):
        # 加载音频文件，并进行预处理
        input_data = self._load_audio(audio_data=audio_data)
        input_data = torch.tensor(input_data.samples, dtype=torch.float32).unsqueeze(0)
        audio_feature = self._audio_featurizer(input_data).to(self.device)
        ...
        # 创建一个字典来存储每个类别的概率
        probabilities = {self.class_labels[i]: round(float(result[i]), 5) for i in range(len(result))}
